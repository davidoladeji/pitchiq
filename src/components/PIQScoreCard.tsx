...
export default function PIQScoreCard({
  ...
  return (
    <div className="bg-white dark:bg-navy-900 rounded-2xl border border-navy-100 dark:border-white/10 shadow-sm p-5 sm:p-6">
      {/* ── Header row: Gauge + Grade + Label ── */}
      <div className="flex items-center gap-4 mb-5">
        {/* Compact gauge ring */}
        <div className="relative w-[100px] h-[100px] shrink-0">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
            <circle cx="50" cy="50" r={gaugeRadius} fill="none" className="stroke-navy-100 dark:stroke-navy-700" strokeWidth={gaugeStroke} />
            <circle
              cx="50"
              cy="50"
              r={gaugeRadius}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-2xl font-bold text-navy dark:text-white tabular-nums">{score.overall}</span>
            <span className="text-[9px] text-navy-500 dark:text-navy-400 uppercase tracking-wider font-medium">PIQ</span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-navy-600 dark:text-navy-300">Fundability Rating</span>
          </div>
      ...
      {/* AI disclaimer */}
      <p className="text-[11px] italic text-navy-400 dark:text-navy-500 mb-4">
      ...
      {detail === "full" ? (
        <>
              ...
              {score.dimensions.map((dim, i) => {
                ...
                return (
                  <text
                    ...
                    className="fill-navy-500 dark:fill-navy-400"
                    fontSize={8}
                    fontWeight={500}
                  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
            {score.dimensions.map((dim) => {
              ...
              return (
                <div
                  key={dim.id}
                  className={`rounded-lg border border-navy-100 dark:border-white/10 border-l-4 ${borderColorClass(dim.score)} bg-navy-50/30 dark:bg-white/5 p-3`}
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    {def && (
                      <svg
                        className="w-4 h-4 text-navy-400 dark:text-navy-500 shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={1.5}
                    ...
                    <span className="text-sm font-semibold text-navy dark:text-white flex-1 truncate">
                      {dim.label}
                    </span>
                    <span className="text-[10px] font-medium text-navy-400 dark:text-navy-500 bg-navy-100 dark:bg-white/10 rounded px-1.5 py-0.5">
                      {dim.weight}%
                    </span>
                    <span
                  ...
                  <p className="text-xs text-navy-500 dark:text-navy-400 leading-relaxed line-clamp-2">
                    {dim.feedback}
                  </p>
                </div>
              );
          ...
          {score.recommendations.length > 0 && (
            <div className="border-t border-navy-100 dark:border-white/10 pt-4">
              <h3 className="text-sm font-semibold text-navy dark:text-white mb-3">Improve your score</h3>
              <div className="flex flex-col gap-2">
                {score.recommendations.map((rec, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2.5 rounded-lg bg-electric/5 dark:bg-electric/10 px-3 py-2.5"
                  >
                    <svg
                    ...
                    <span className="text-sm text-navy-600 dark:text-navy-300 leading-snug">{rec}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-center blur-[6px] select-none pointer-events-none" aria-hidden="true">
              ...
              >
                ...
                {Array.from({ length: NUM_AXES }).map((_, i) => {
                  const [x, y] = polarToXY(cx, cy, maxR, i);
                  return (
                    <line key={i} x1={cx} y1={cy} x2={x} y2={y} className="stroke-navy-200 dark:stroke-navy-700" strokeWidth={0.5} />
                  );
                })}
                <polygon
            ...
            <div className="relative mb-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 blur-[6px] select-none pointer-events-none" aria-hidden="true">
                {score.dimensions.slice(2, 6).map((dim) => (
                  <div
                    key={dim.id}
                    className="rounded-lg border border-navy-100 dark:border-white/10 border-l-4 border-l-navy-300 bg-navy-50/30 dark:bg-white/5 p-3"
                  >
                    <div className="flex items-center gap-2 mb-1.5">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <Link
                  href="/#pricing"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-electric/10 text-electric text-xs font-semibold hover:bg-electric/20 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-navy-950"
          ...
          <div className="border-t border-navy-100 dark:border-white/10 pt-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-navy-500 dark:text-navy-400">
                <Link href="/#pricing" className="text-electric font-semibold hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-navy-950 rounded">
                  Upgrade to Pro
                </Link>{" "}
